...
%   wf(kf,1) = weights for GMM f
%   mg(kg,p) = mixture means for GMM g
%   vg(kg,p) or vg(p,p,kg) variances (diagonal or full) for GMM g
%   wg(kg,1) = weights for GMM g
%
% Outputs:
%   d             the approximate KL divergence D(f||g)
%   klfg(kf,kg)   the exact KL divergence between the components of f and g
%                 (if only 3 input arguments are given, this is the KL divergence
%                  between the comonents of f and the diagonal will be zero)
%
...
% from [2] that is exact when they are single component gaussians and that is zero
% if p=q. However, the approximation may be negative if p~=q.
%
% Refs:
...
%      Version: $Id: gaussmixk.m 3229 2013-07-04 16:36:20Z dmb $
%
